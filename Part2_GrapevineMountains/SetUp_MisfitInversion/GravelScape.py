...
import GravelScape_SandScape_Functions
...
class GravelGSN:
    ...
    def run_step(self,endtime): #Run_Step function only takes into accout at one time step
        ...
        EE=GravelScape_SandScape_Functions.sum_stack(self.stack,self.rec,flatE,self.nrec,self.rec_weights) #Calculate the erosional flux through the nodes in the stack
        ...
        Y, D0Org=GravelScape_SandScape_Functions.qD0sum_stack(self.stack,self.rec,RR,flux,self.nrec,self.rec_weights,D0T) #R is combined through the stack to compute Y*. Fedele and Paola (2007) use an integration of Y* through the drainage network in the computation of grain size fining. 
